...
data "aws_s3_object" "lambda_package" {
  ...
  key    = "lambda/test2/test2.zip"  # We organize functions in their own folders
}
...
resource "aws_lambda_function_event_invoke_config" "test2_retry" {
...
resource "aws_cloudwatch_log_group" "lambda_log_group" {
  name              = "/aws/lambda/${aws_lambda_function.test2.function_name}"
