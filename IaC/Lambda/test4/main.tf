...
data "aws_s3_object" "lambda_package" {
  ...
  key    = "lambda/test4/test4.zip"  # We organize functions in their own folders
}

# Lambda Function with S3 deployment
resource "aws_lambda_function" "test4" {
...
resource "aws_lambda_function_event_invoke_config" "test4_retry" {
  function_name                = aws_lambda_function.test4.function_name
...
resource "aws_cloudwatch_log_group" "lambda_log_group" {
  name              = "/aws/lambda/${aws_lambda_function.test4.function_name}"
